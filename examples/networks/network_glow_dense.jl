...
n_in = 1    # if 1 then split_scales is automatically activated to increase channel for coupling layer
n_hidden = 32
batchsize = 10
...
# Glow network
G = NetworkGlow(n_in, n_hidden, L, K; dense=true, nx=nx, ndims=1)  |> device

# Normalizing flow objective function
function loss(X)
