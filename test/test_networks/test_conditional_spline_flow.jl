...
@testset "round trip and per-sample log-determinant" begin
    ...
    @test size(Z) == size(X)
    @test all(-B .<= Z .<= B)                            # still a bijection of the box
...
@testset "the unconditional path is unchanged" begin
    net = perturb!(InvertibleChain(ntuple(i ->
        CouplingLayerSpline(d, 8; mix = false, swap = isodd(i), bound = B, logdet = true), 4)))
    w = randn(Float32, N)
...
@testset "mixed chain: layers with no conditioner ignore the context" begin
    net = InvertibleChain(
        CouplingLayerSpline(d, 8; n_ctx = nc, mix = false, bound = B, logdet = true),
        SplineLayer(d; bound = B, logdet = true),
        CouplingLayerSpline(d, 8; n_ctx = nc, mix = false, swap = true, bound = B, logdet = true))
    @test preserves_box(net, B)
    perturb!(net)
...
@testset "misuse is rejected loudly" begin
    ...
    plain = InvertibleChain(SplineLayer(d; bound = B, logdet = true))
